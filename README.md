...
Upon inspection of trained models, you may find that some fits need to be adjusted. The function `GMMfit()` accepts three optional parameters that can be used to tweak model fitting:
...
For example, here's a figure that shows the original fit made with default parameters (left) against a minor adjustment of the inter-quantile range (right).

![](docs/Fig2.png)

More explicitly, the fit on the left was generated by 

``` r
F1 <- GMMfit( X, CellId, "Cell_25546ONCD45" )
plotFit( F1, "Cell_25546ONCD45" )
```

while the fit on the right was generated by

``` r
F2 <- GMMfit( X, CellId, "Cell_25546ONCD45", qq=0.005 )
plotFit( F2, "Cell_25546ONCD45" )
```

If you want to replace the original fit in `Fits` with the modified one from `F2`, this can be done by dropping the row corresponding to CD45 in the original data frame and simply concatenating the new data frame:

``` r
Fits <- filter( Fits, Marker != "Cell_25546ONCD45" )    # Keep all non-CD45 rows
Fits <- bind_rows( Fits, F2 )                           # Append the rows from F2 to Fits
```
